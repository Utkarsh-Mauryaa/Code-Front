...
export const createInterview = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const interview = await ctx.db.insert("interviews", args);
        return interview;
    }
})
...
export const updateInterviewStatus = mutation({
    args: {
        id: v.id("interviews"), // when interview will be created convex will give a unique id to that interview automatically
        status: v.string(),
    },
    handler: async (ctx, args) => {
      return await ctx.db.patch(args.id, {
        status: args.status,
        ...(args.status === "completed" ? {endTime: Date.now()} : {}) // if status is completed then add endTime
      })
    }
})
